...
This package implements the gene expression anti-profiles method in \cite{ap}. In this vignette we will use the companion \Rpackage{antiProfilesData} package to illustrate some of the analysis in that paper. 
...
hist(colonStats@probes$stat, nc=100, 
